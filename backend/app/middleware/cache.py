    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if isinstance(result, tuple):
                ...
                if status_code == 200:
                    # Extract JSON data from Response object before caching
                    data = response_obj.get_json()
                    query_cache.set(cache_key, data, ttl)
                # Return the original response object with cache header
                response_obj.headers['X-Cache'] = 'MISS'
                return response_obj
            else:
                # Handle non-tuple responses (shouldn't happen with current routes)
