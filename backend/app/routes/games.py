...
def start_game(game_id):
    ...
    # Calculate winnings (85% of pot, 15% house cut) - only for games with stakes
    if float(game.total_pot) > 0:
        ...
        # Free game - just mark winner/loser without payout
        winner.result = 'win'
...
def cancel_game(game_id):
    ...
    # Refund all stakes (only for games with stakes)
    entries = GameEntry.query.filter_by(game_id=game_id).all()
    ...
    for entry in entries:
        if entry.stake_amount > 0:
            ...
            db.session.add(wallet_tx)
    # For 0 stake games, no refund needed
