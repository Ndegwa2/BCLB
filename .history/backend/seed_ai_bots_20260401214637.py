...
def create_ai_bots():
    ...
    with app.app_context():
        ...
        existing_difficulties = [bot.ai_difficulty for bot in existing_ai]
        ...
        for bot_data in ai_bots:
            # Skip if this difficulty already exists
            if bot_data['ai_difficulty'] in existing_difficulties:
                print(f"AI bot for difficulty '{bot_data['ai_difficulty']}' already exists. Skipping.")
                continue
        ...
        if created_count > 0:
            db.session.commit()
            print(f"Created {created_count} new AI bot(s) successfully!")
        else:
            print("All AI bots already exist. No new bots created.")
