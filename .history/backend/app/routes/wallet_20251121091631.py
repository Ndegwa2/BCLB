...
def get_wallet():
    ...
    # Sum of all credit transactions minus debit transactions
    ...
    locked_credit_sum = db.session.query(func.sum(WalletTransaction.amount)).filter(
        WalletTransaction.user_id == user.id,
        WalletTransaction.direction == 'credit',
        WalletTransaction.status == 'pending'
    ).scalar() or 0

    locked_debit_sum = db.session.query(func.sum(WalletTransaction.amount)).filter(
        WalletTransaction.user_id == user.id,
        WalletTransaction.direction == 'debit',
        WalletTransaction.status == 'pending'
    ).scalar() or 0

    locked_balance = float(locked_credit_sum - locked_debit_sum)

    # Get recent transactions with pagination
    transactions_query = WalletTransaction.query.filter_by(user_id=user.id).order_by(
        WalletTransaction.created_at.desc()
    )

    transactions = transactions_query.paginate(page=page, per_page=limit, error_out=False)
