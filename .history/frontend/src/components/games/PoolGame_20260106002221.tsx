...
class PoolGameScene extends Phaser.Scene {
  ...
  private createPoolTable() {
    ...
    // SVG reference dimensions: 1000x550
    const svgW = 1000;
    const svgH = 550;

    // Wood Frame (outermost layer) - dark brown #3e2723
    const frameGraphics = this.add.graphics();
    frameGraphics.fillStyle(0x3e2723, 1);
    frameGraphics.fillRoundedRect(0, 0, w, h, 32 * (w / svgW));

    // Rails (middle layer) - medium brown #4e342e
    const railsGraphics = this.add.graphics();
    railsGraphics.fillStyle(0x4e342e, 1);
    railsGraphics.fillRoundedRect(
      w * (35 / svgW),
      h * (35 / svgH),
      w * (930 / svgW),
      h * (480 / svgH),
    ...
    // Felt (playing surface) - green #2e7d32
    const feltGraphics = this.add.graphics();
    feltGraphics.fillStyle(0x2e7d32, 1);
    feltGraphics.fillRoundedRect(
      w * (75 / svgW),
      h * (75 / svgH),
      w * (850 / svgW),
      h * (400 / svgH),
  ...
  private createDiamondMarkers() {
    const w = this.scale.width;
    const h = this.scale.height;
    const svgW = 1000;
    const svgH = 550;
    const markerRadius = 4 * (w / svgW);
    const markerColor = 0xd4af37; // Gold

    // Top rail markers (y = 55 in SVG)
    const topMarkerY = h * (55 / svgH);
    const topMarkerXPositions = [220, 390, 560, 730];

    topMarkerXPositions.forEach(x => {
      this.add.circle(w * (x / svgW), topMarkerY, markerRadius, markerColor);
    });

    // Bottom rail markers (y = 495 in SVG)
    const bottomMarkerY = h * (495 / svgH);
    const bottomMarkerXPositions = [220, 390, 560, 730];

    bottomMarkerXPositions.forEach(x => {
      this.add.circle(w * (x / svgW), bottomMarkerY, markerRadius, markerColor);
    });

    console.log('Diamond markers created');
  }

  private createTableWalls() {
    // Define table boundaries at the outermost layer (wood frame edges)
    // Using the wood frame dimensions for better portability
    const tableLeft = 0;                        // Left edge of wood frame
    const tableRight = this.scale.width;        // Right edge of wood frame
    const tableTop = 0;                         // Top edge of wood frame
    const tableBottom = this.scale.height;      // Bottom edge of wood frame
    const wallThickness = 15;                   // Thicker walls for better boundaries

    // Top wall - at the very top of wood frame
    ...
    // Bottom wall - at the very bottom of wood frame
    this.matter.add.rectangle(
      (tableLeft + tableRight) / 2,
    ...
    // Left wall - at the very left of wood frame
    this.matter.add.rectangle(
    ...
    // Right wall - at the very right of wood frame
    this.matter.add.rectangle(
    ...
    console.log('Table walls created at outermost layer (wood frame edges)');
  }
  ...
  private createPockets() {
    ...
    // Corner pocket radius (r="24" in new SVG)
    const cornerPocketRadius = 24 * (w / svgW);
    // Side pocket radius (r="20" in new SVG)
    const sidePocketRadius = 20 * (w / svgW);

    // Pocket positions matching new SVG coordinates
    const pocketPositions = [
      // Corner pockets (r=24)
      { x: w * (75 / svgW), y: h * (75 / svgH), radius: cornerPocketRadius },     // Top left
      { x: w * (925 / svgW), y: h * (75 / svgH), radius: cornerPocketRadius },    // Top right
      { x: w * (75 / svgW), y: h * (475 / svgH), radius: cornerPocketRadius },    // Bottom left
      { x: w * (925 / svgW), y: h * (475 / svgH), radius: cornerPocketRadius },   // Bottom right
      // Side pockets (r=20)
      { x: w * (500 / svgW), y: h * (75 / svgH), radius: sidePocketRadius },      // Top center
      { x: w * (500 / svgW), y: h * (475 / svgH), radius: sidePocketRadius }      // Bottom center
    ];

    pocketPositions.forEach((pos, index) => {
      // Create visible pocket (black circle)
      const pocket = this.add.circle(pos.x, pos.y, pos.radius, pocketColor);
      pocket.setDepth(-1);
      (this.pockets as any).push(pocket);

      // Add physics sensor for ball detection with corrected collision filter
      // Sensors should detect all balls (category 0x0001) without bouncing them
      this.matter.add.circle(pos.x, pos.y, pos.radius * 0.85, {
        isSensor: true,
        label: `pocket-${index}`,
        collisionFilter: {
          category: 0x0002, // Pocket category
          mask: 0x0001,     // Collide with balls only
          group: 0
        }
      });

      console.log(`Pocket ${index} created at:`, { x: pos.x, y: pos.y, radius: pos.radius });
    });

    console.log('Pockets created matching enhanced SVG design with diamond markers');
  }
  ...
  private createBalls() {
    ...
    // Define safe boundary for ball placement (outermost layer with padding)
    ...
    // Position cue ball within safe boundaries (right side for break shot)
    ...
    cueBall.setFriction(0.02); // Slightly higher friction for stability
    cueBall.setFrictionAir(0.01); // Reduced air resistance
    (cueBall.body as any).restitution = 0.8; // Reduced bounce for stability
    cueBall.setMass(1);
    cueBall.setBounce(0.8);
    cueBall.setCollisionCategory(0x0001);
    cueBall.setCollidesWith(0x0001); // Only collide with balls, not pockets for performance
    ...
    ballTypes.forEach((ballConfig, index) => {
      ...
      ball.setFriction(0.03); // Slightly higher friction for stability
      (ball.body as any).restitution = 0.7; // Reduced bounce for better performance
      ball.setMass(1);
      ball.setCollisionCategory(0x0001);
      ball.setCollidesWith(0x0001); // Only collide with balls for performance
  ...
  private validateBallBoundaries() {
    // Optimized boundary validation - reduced frequency and simplified checks
    const safeLeft = 50;
    ...
    // Only check balls that are likely to be out of bounds (higher speed balls)
    ...
    fastBalls.forEach(ball => {
      ...
      // Optimized boundary checks with early exits
      if (x < safeLeft + ballRadius || x > safeRight - ballRadius) {
        x = Math.min(Math.max(x, safeLeft + ballRadius), safeRight - ballRadius);
        needsReposition = true;
      }

      if (y < safeTop + ballRadius || y > safeBottom - ballRadius) {
        y = Math.min(Math.max(y, safeTop + ballRadius), safeBottom - ballRadius);
        needsReposition = true;
      }
  ...
  private createUI() {
    ...
    // Pocketed balls display bar (top right)
  ...
  private createPocketedBallsDisplay() {
    // Create container for pocketed balls display
    this.pocketedBallsDisplay = this.add.container(this.scale.width - 180, 20);
    
    // Background for display bar
    const bgGraphics = this.add.graphics();
    bgGraphics.fillStyle(0x000000, 0.7);
    bgGraphics.fillRoundedRect(0, 0, 170, 80, 8);
    this.pocketedBallsDisplay.add(bgGraphics);
    
    // Title text
    const titleText = this.add.text(85, 15, 'PORTED', {
      fontSize: '14px',
      color: '#FFD700',
      fontStyle: 'bold'
    }).setOrigin(0.5);
    ...
    // Container for ball icons
    const ballsContainer = this.add.container(0, 35);
    this.pocketedBallsDisplay.add(ballsContainer);
    
    // Store reference to update later
    (this.pocketedBallsDisplay as any).ballsContainer = ballsContainer;
    this.pocketedBallsDisplay.setDepth(100); // Ensure it's on top
  }
  ...
  private updatePocketedBallsDisplay() {
    ...
    // Create ball icons for pocketed balls
    const ballRadius = 12;
    const spacing = 28;
    const maxBallsPerRow = 5;
    ...
    this.pocketedBalls.forEach((ball, index) => {
      ...
      // Determine color based on ball type
      let ballColor = 0xFFFFFF;
      if (ball.type === 'solid') ballColor = 0xFFD700; // Yellow for solids
      else if (ball.type === 'stripe') ballColor = 0xFF0000; // Red for stripes
      else if (ball.type === 'eight') ballColor = 0x000000; // Black for 8-ball
      
      // Draw ball
      ballIcon.fillStyle(ballColor, 1);
      ballIcon.fillCircle(x + ballRadius, y + ballRadius, ballRadius);
      
      // Add stripe pattern for stripe balls
      if (ball.type === 'stripe') {
        ballIcon.fillStyle(0xFFFFFF, 1);
        ballIcon.fillRect(x + ballRadius - 5, y, 10, ballRadius * 2);
      }
      
      // Add border
      ballIcon.lineStyle(1, 0xCCCCCC, 1);
      ballIcon.strokeCircle(x + ballRadius, y + ballRadius, ballRadius);
      
      // Add number text
      const numberText = this.add.text(x + ballRadius, y + ballRadius, ball.number.toString(), {
        fontSize: '10px',
        color: ball.type === 'eight' || ball.type === 'stripe' ? '#FFFFFF' : '#000000',
        fontStyle: 'bold'
      }).setOrigin(0.5);
      
      ballsContainer.add(ballIcon);
      ballsContainer.add(numberText);
    });
    
    // Update background height if needed
    const numRows = Math.ceil(this.pocketedBalls.length / maxBallsPerRow);
    const newHeight = 55 + numRows * 26;
    if (newHeight > 80) {
      // Clear and redraw background
      const bgGraphics = this.pocketedBallsDisplay.getAt(0) as Phaser.GameObjects.Graphics;
      if (bgGraphics) {
        bgGraphics.clear();
        bgGraphics.fillStyle(0x000000, 0.7);
        bgGraphics.fillRoundedRect(0, 0, 170, newHeight, 8);
      }
    }
  }
  ...
  private setupEventListeners() {
    ...
    // Optimized collision detection - reduce frequency and add throttling
    let lastCollisionTime = 0;
    const collisionThrottle = 50; // Reduced throttle for better pocket detection
    
    this.matter.world.on('collisionstart', (event: any) => {
      const now = Date.now();
      if (now - lastCollisionTime < collisionThrottle) return; // Throttle collision events
      lastCollisionTime = now;
      
      console.log('Collisionstart event fired:', event.pairs.length);
      this.handleCollisionStart(event);
      this.handleBallPocketCollisions(event);
    }, this);
    
    // Also use 'collisionactive' for continuous pocket detection (balls moving through sensor)
    this.matter.world.on('collisionactive', (event: any) => {
      // Process active collisions but less frequently
      if (Date.now() % 2 !== 0) return;
      this.handlePocketSensorActive(event);
    }, this);
    
    this.matter.world.on('collisionend', (event: any) => {
      if (Date.now() % 5 !== 0) return; // Only process every 5th frame
      this.handleCollisionEnd(event);
    }, this);
  }

  private handlePocketSensorActive(event: any) {
    // Handle continuous collision with pocket sensors (for balls moving through)
    event.pairs.forEach((pair: any) => {
      const { bodyA, bodyB } = pair;
      
      // Check if ball is colliding with pocket sensor
      if (bodyA.label && bodyA.label.startsWith('pocket-')) {
        const ball = bodyB.gameObject;
        if (ball && ball !== this.cueBall && ball.active && ball.getData('type') !== 'cue') {
          // Ball is inside pocket sensor - swallow it!
  ...
  private shootCueBall(pointer: Phaser.Input.Pointer) {
    ...
    // Apply force based on power - doubled multiplier for increased cue stick power
    const force = this.shotPower * 0.2; // Increased from 0.1 to 0.2 to double power
  ...
  private swallowBall(ball: Phaser.Physics.Matter.Sprite) {
    ...
    console.log(`Swallowing ball ${ballNumber} (${ballType})`);
    
    // Create swallow effect at ball position
    const swallowEffect = this.add.graphics();
    
    // Expanding ring effect
    swallowEffect.lineStyle(3, 0xFFFFFF, 1);
    swallowEffect.strokeCircle(ball.x, ball.y, 20);
    
    // Animate the swallow effect
    this.tweens.add({
    ...
    // Play swallow/potting sound
    this.playPottingSound();
    
    console.log(`Ball ${ballNumber} swallowed successfully`);
  }

  private animateBallPocket(ball: Phaser.Physics.Matter.Sprite) {
    // Optimized ball pocket animation - reduced complexity
    ...
    console.log(`Optimized ball slide to pocket:`, { pocketX, pocketY });

    // Calculate direction and distance (optimized)
    ...
    // Simplified particle trail (only create if needed)
    if (distanceToPocket > 50) {
      this.createBallTrail(ball, pocketX, pocketY);
    }

    // Optimized physics - reduced update frequency
    const slideForce = 0.12; // Reduced force
    const initialVelocity = slideForce * Math.sqrt(distanceToPocket) * 0.015; // Reduced multiplier
    ...
    // Simplified spin - only set once
    ball.setAngularVelocity((Math.random() - 0.5) * 0.3);

    // Optimized animation with throttled updates
    let lastUpdate = 0;
    const updateInterval = 32; // ~30fps instead of 60fps
    const totalSlideTime = 800; // Reduced duration
    ...
    const slideUpdate = () => {
      ...
      if (velocity) {
        ball.setVelocity(velocity.x * 0.96, velocity.y * 0.96); // Fixed friction
      }

      // Check if ball is close enough to pocket
      const currentDistance = Phaser.Math.Distance.Between(ball.x, ball.y, pocketX, pocketY);
      const ballSpeed = velocity ? Math.sqrt(velocity.x * velocity.x + velocity.y * velocity.y) : 0;

      if (currentDistance < ballRadius * 1.2 || ballSpeed < 0.01) {
        this.slideBallOffTable(ball, pocketX, pocketY);
        clearInterval(slideInterval);
      }
    };

    // Use setInterval instead of per-frame updates
    const slideInterval = setInterval(slideUpdate, 16); // 60fps but logic only runs every 32ms

    this.playBallSlideSound();
  }

  private findClosestPocket(ballX: number, ballY: number): { x: number, y: number } | null {
    // Updated pocket positions to match outermost layer configuration
    const pocketPositions = [
      { x: 75, y: 75 },  // Top left (adjusted for wood frame)
      { x: this.scale.width * 0.5, y: 75 }, // Top center
      { x: this.scale.width - 75, y: 75 },  // Top right
      { x: 75, y: this.scale.height - 75 }, // Bottom left
      { x: this.scale.width * 0.5, y: this.scale.height - 75 }, // Bottom center
      { x: this.scale.width - 75, y: this.scale.height - 75 }  // Bottom right
    ];
  ...
  private createBallTrail(ball: Phaser.Physics.Matter.Sprite, targetX: number, targetY: number) {
    // Optimized particle trail - reduced particle count and simplified effects
    if (!ball.active) return;
    
    const trailParticles: any[] = [];
    const trailLength = 5; // Reduced from 10 to 5
    const maxDuration = 300; // Reduced from 400+ to 300

    for (let i = 0; i < trailLength; i++) {
      const size = Math.max(2, 4 - i * 0.3);
      const alpha = Math.max(0.2, 0.6 - i * 0.08);
      
      const particle = this.add.circle(ball.x, ball.y, size, 0xFFFFFF, alpha);
      particle.setDepth(-2);
      
      trailParticles.push(particle);

      // Simplified fade out without bounce effect
      this.tweens.add({
        targets: particle,
        alpha: 0,
        scale: 0.2,
    ...
    // Optimized trail position updates - reduced frequency
    let trailIndex = 0;
    const updateInterval = setInterval(() => {
      ...
      if (particle && particle.active) {
        // Reduced randomness for performance
        const jitterX = (Math.random() - 0.5) * 4;
        particle.setPosition(ball.x + jitterX, ball.y);
      }
      trailIndex++;
    }, 50); // Increased interval from 16ms to 50ms (20fps instead of 60fps)
  ...
  update() {
    ...
    // Only validate boundaries every 3 frames instead of every frame
    if (this.frameCount - this.lastBoundaryCheck >= 3) {
      this.validateBallBoundaries();
      this.lastBoundaryCheck = this.frameCount;
    }

    // Only check for pockets every 2 frames and only when balls are moving
    const ballsMoving = this.balls.some(ball => ball.active && (ball.body as any).speed > 0.1);
    if (ballsMoving && this.frameCount - this.lastPocketCheck >= 2) {
      this.checkDistanceBasedPocketDetection();
      this.lastPocketCheck = this.frameCount;
    }

    // Update game logic each frame (keep this as is for game flow)
    if (this.cueBall && (this.cueBall.body as any).speed < 0.1 && !this.isAiming) {
      ...
      if (this.gameStarted && !this.gameOverText) {
        ...
        const ballsMovingCheck = this.balls.some(ball => (ball.body as any).speed > 0.1);
    ...
    // Update power indicator if aiming (keep this as is)
    if (this.isAiming && this.powerIndicator) {
      // Make power indicator pulse (reduced frequency)
  ...
  private checkDistanceBasedPocketDetection() {
    // Optimized pocket detection - cache positions and pre-calc distances
    ...
    // Early exit if no balls are moving fast enough
    const fastBalls = this.balls.filter(ball => ball.active && (ball.body as any).speed > 0.5);
    if (fastBalls.length === 0) return;

    fastBalls.forEach(ball => {
      if (!ball || ball.getData('type') === 'cue') return;
      ...
      // Optimized distance checks - use squared distances to avoid sqrt
      for (let i = 0; i < this.pocketPositions!.length; i++) {
        ...
        // If ball is very close to pocket (within 35 pixels), trigger potting
        if (squaredDistance < squaredPocketRadius) {
          console.log(`🎱 POCKET DETECTION: Ball ${ballNumber} (${ballType}) detected near pocket ${i}! Distance: ${Math.sqrt(squaredDistance).toFixed(2)}`);
          this.handleBallPocketed(ball);
          break;
        }
      }
