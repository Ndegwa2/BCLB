...
class PoolGameScene extends Phaser.Scene {
  ...
  private createPockets() {
    ...
    pocketPositions.forEach((pos, index) => {
      ...
      this.matter.add.circle(pos.x, pos.y, pos.radius * 0.9, {
        isSensor: true,
        label: `pocket-${index}`,
        collisionFilter: {
          category: 0x0002,
  ...
  private setupEventListeners() {
    ...
    const collisionThrottle = 100; // ms between collision events
    ...
    // Reduce frequency of active and end collision events
    this.matter.world.on('collisionactive', (event: any) => {
      if (Date.now() % 3 !== 0) return; // Only process every 3rd frame
