...
class PoolGameScene extends Phaser.Scene {
  ...
  private playerGroups: { player1: 'solids' | 'stripes' | null, player2: 'solids' | 'stripes' | null } = { player1: null, player2: null };
  private currentPlayerGroup: 'solids' | 'stripes' | null = null;
  private ballsRemaining: { solids: number, stripes: number, eight: number } = { solids: 7, stripes: 7, eight: 1 };
  ...
  private createBalls() {
    ...
    // Create 15 balls for 8-ball pool (1-7 solids, 9-15 stripes, 8-ball black)
    // Using proper 8-ball pool ball colors
    const ballTypes = [
      { key: 'ball1', type: 'solid', number: 1, color: 0xFFFF00 }, // Yellow (solid)
      { key: 'ball2', type: 'solid', number: 2, color: 0x0000FF }, // Blue (solid)
      { key: 'ball3', type: 'solid', number: 3, color: 0xFF0000 }, // Red (solid)
      { key: 'ball4', type: 'solid', number: 4, color: 0x800080 }, // Purple (solid)
      { key: 'ball5', type: 'solid', number: 5, color: 0xFFA500 }, // Orange (solid)
      { key: 'ball6', type: 'solid', number: 6, color: 0x008000 }, // Green (solid)
      { key: 'ball7', type: 'solid', number: 7, color: 0x800000 }, // Maroon (solid)
      { key: 'ball8', type: 'eight', number: 8, color: 0x000000 }, // Black (8-ball)
      { key: 'ball9', type: 'stripe', number: 9, color: 0xFFFF00 }, // Yellow (stripe)
      { key: 'ball10', type: 'stripe', number: 10, color: 0x0000FF }, // Blue (stripe)
      { key: 'ball11', type: 'stripe', number: 11, color: 0xFF0000 }, // Red (stripe)
      { key: 'ball12', type: 'stripe', number: 12, color: 0x800080 }, // Purple (stripe)
      { key: 'ball13', type: 'stripe', number: 13, color: 0xFFA500 }, // Orange (stripe)
      { key: 'ball14', type: 'stripe', number: 14, color: 0x008000 }, // Green (stripe)
      { key: 'ball15', type: 'stripe', number: 15, color: 0x800000 }, // Maroon (stripe)
    ];
  ...
  private getBallRackPositions(): {x: number, y: number}[] {
    ...
    // Row 4 (4 balls)
    positions.push({
      x: centerX - ballSpacing * 1.5,
      y: centerY
    });
    positions.push({
      x: centerX - ballSpacing * 0.5,
      y: centerY
    });
    positions.push({
      x: centerX + ballSpacing * 0.5,
  ...
  private determinePlayerGroups() {
    ...
    this.playerGroups.player1 = solidsPlayer === 0 ? 'solids' : 'stripes';
    this.playerGroups.player2 = solidsPlayer === 0 ? 'stripes' : 'solids';
