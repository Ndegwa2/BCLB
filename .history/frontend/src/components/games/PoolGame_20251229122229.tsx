...
class PoolGameScene extends Phaser.Scene {
  ...
  private createCue(pointer: Phaser.Input.Pointer) {
    ...
    // Create cue stick as a rectangle (pool cue)
    const cueLength = 200;
    const cueWidth = 8;
    
    // Create cue stick graphics
    const cueGraphics = this.add.graphics();
    
    // Cue stick body (brown wood)
    cueGraphics.fillStyle(0x8B4513, 1);
    cueGraphics.fillRect(0, 0, cueLength, cueWidth);
    
    // Cue tip (white)
    cueGraphics.fillStyle(0xFFFFFF, 1);
    cueGraphics.fillRect(0, 0, 10, cueWidth);
    
    // Cue handle (darker brown)
    cueGraphics.fillStyle(0x5D3A1A, 1);
    cueGraphics.fillRect(cueLength - 40, 0, 40, cueWidth);
    
    // Generate texture
    cueGraphics.generateTexture('cueStickTexture', cueLength, cueWidth);
    cueGraphics.destroy();

    // Create cue stick sprite
    this.cueStick = this.add.image(this.cueBall.x, this.cueBall.y, 'cueStickTexture');
    this.cueStick.setOrigin(0, 0.5);

    // Create aiming line
    this.cue = this.add.line(0, 0, 0, 0, 0xFFFFFF, 0.5);
