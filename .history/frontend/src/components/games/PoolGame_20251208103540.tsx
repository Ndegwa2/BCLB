...
class PoolGameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // Load pool table assets
    try {
      this.load.image('poolTable', '/assets/pool/pool-table.png');
      this.load.image('cueBall', '/assets/pool/cue-ball.png');
      this.load.image('solid1', '/assets/pool/solid-1.png');
      this.load.image('solid2', '/assets/pool/solid-2.png');
      this.load.image('solid3', '/assets/pool/solid-3.png');
      this.load.image('solid4', '/assets/pool/solid-4.png');
      this.load.image('solid5', '/assets/pool/solid-5.png');
      this.load.image('solid6', '/assets/pool/solid-6.png');
      this.load.image('solid7', '/assets/pool/solid-7.png');
      this.load.image('stripe9', '/assets/pool/stripe-9.png');
      this.load.image('stripe10', '/assets/pool/stripe-10.png');
      this.load.image('stripe11', '/assets/pool/stripe-11.png');
      this.load.image('stripe12', '/assets/pool/stripe-12.png');
      this.load.image('stripe13', '/assets/pool/stripe-13.png');
      this.load.image('stripe14', '/assets/pool/stripe-14.png');
      this.load.image('stripe15', '/assets/pool/stripe-15.png');
      this.load.image('eightBall', '/assets/pool/8-ball.png');
      this.load.image('cueStick', '/assets/pool/cue-stick.png');

      // Load fallback assets if custom ones aren't available
  ...
  create() {
    console.log('Creating PoolGameScene');
  ...
  private createBalls() {
    ...
    this.cueBall = this.matter.add.image(
      this.scale.width * 0.75,
      this.scale.height * 0.5,
      'cueBall',
      null,
      { shape: 'circle', radius: ballRadius }
    );
    this.cueBall.setCircle(ballRadius);
    this.cueBall.setFriction(0.02);
    this.cueBall.setRestitution(0.8);
    this.cueBall.setMass(1);
    this.cueBall.setCollisionCategory(0x0001);
    this.cueBall.setCollidesWith(0x0001);
    this.cueBall.setData('type', 'cue');
    this.cueBall.setData('number', 0);

    this.balls.push(this.cueBall);

    // Create 15 colored balls (7 solids, 7 stripes, 1 eight-ball)
    const ballTypes = [
      { key: 'solid1', type: 'solid', number: 1 },
      { key: 'solid2', type: 'solid', number: 2 },
      { key: 'solid3', type: 'solid', number: 3 },
      { key: 'solid4', type: 'solid', number: 4 },
      { key: 'solid5', type: 'solid', number: 5 },
      { key: 'solid6', type: 'solid', number: 6 },
      { key: 'solid7', type: 'solid', number: 7 },
      { key: 'stripe9', type: 'stripe', number: 9 },
      { key: 'stripe10', type: 'stripe', number: 10 },
      { key: 'stripe11', type: 'stripe', number: 11 },
      { key: 'stripe12', type: 'stripe', number: 12 },
      { key: 'stripe13', type: 'stripe', number: 13 },
      { key: 'stripe14', type: 'stripe', number: 14 },
      { key: 'stripe15', type: 'stripe', number: 15 },
      { key: 'eightBall', type: 'eight', number: 8 }
    ];
  ...
  update() {
    ...
    if (this.cueBall && this.cueBall.body.speed < 0.1 && !this.isAiming) {
      ...
      if (this.gameStarted && !this.gameOverText) {
        ...
        const ballsMoving = this.balls.some(ball => ball.body.speed > 0.1);
