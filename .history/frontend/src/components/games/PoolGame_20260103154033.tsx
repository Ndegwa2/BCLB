...
class PoolGameScene extends Phaser.Scene {
  ...
  private animateBallPocket(ball: Phaser.Physics.Matter.Sprite) {
    ...
    // Apply initial force toward pocket
    ball.setVelocity(
      Math.cos(angleToPocket) * slideForce * distanceToPocket * 0.01,
      Math.sin(angleToPocket) * slideForce * distanceToPocket * 0.01
    );

    // Add angular velocity for rolling effect
    ball.setAngularVelocity(spinSpeed * (Math.random() > 0.5 ? 1 : -1));
    ...
    const slideUpdate = () => {
      ...
      if (Math.random() < 0.3) {
        ball.setVelocity(
          ball.body.velocity.x + (Math.random() - 0.5) * 0.02,
          ball.body.velocity.y + (Math.random() - 0.5) * 0.02
        );
      }

      // Check if ball is close enough to pocket
      const currentDistance = Phaser.Math.Distance.Between(ball.x, ball.y, pocketX, pocketY);
      if (currentDistance < ballRadius * 1.5 || slideProgress >= 1) {
  ...
  private createBallTrail(ball: Phaser.Physics.Matter.Sprite, targetX: number, targetY: number) {
    // Create particle trail effect behind the sliding ball
    const trailParticles: Phaser.GameObjects.Rectangle[] = [];
    const trailLength = 8;
    const trailSpacing = 3;

    for (let i = 0; i < trailLength; i++) {
      const trailParticle = this.add.rectangle(ball.x, ball.y, 4, 4, 0xFFFFFF, 0.6);
      trailParticle.setDepth(-2);
    ...
    // Update trail positions during slide
    let trailIndex = 0;
    const trailUpdate = () => {
    ...
    this.events.on('update', trailUpdate);
  }
