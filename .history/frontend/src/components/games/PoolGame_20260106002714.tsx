...
class PoolGameScene extends Phaser.Scene {
  ...
  private ballsMovingCheckScheduled = false;
  ...
  private findClosestPocket(ballX: number, ballY: number): { x: number, y: number } | null {
    // Pre-computed pocket positions for performance
    if (!this.pocketPositions) {
      this.pocketPositions = [
        { x: 75, y: 75 },
        { x: this.scale.width * 0.5, y: 75 },
        { x: this.scale.width - 75, y: 75 },
        { x: 75, y: this.scale.height - 75 },
        { x: this.scale.width * 0.5, y: this.scale.height - 75 },
        { x: this.scale.width - 75, y: this.scale.height - 75 }
      ];
    }
  ...
  private checkDistanceBasedPocketDetection() {
    // Optimized pocket detection with caching
    if (!this.pocketPositions) {
      this.pocketPositions = [
        { x: 75, y: 75 },
        { x: this.scale.width * 0.5, y: 75 },
    ...
    // Early exit if no balls are moving
