...
class PoolGameScene extends Phaser.Scene {
  ...
  private createPowerIndicator(pointer: Phaser.Input.Pointer) {
    ...
    // Add text label
    const powerText = this.add.text(this.cueBall.x, this.cueBall.y - 40, 'Power', {
      fontSize: '14px',
      color: '#FFFFFF',
      backgroundColor: '#00000080',
      padding: { x: 5, y: 2 }
    }).setOrigin(0.5);
    powerText.setData('type', 'powerText');
  ...
  private updatePowerIndicator(pointer: Phaser.Input.Pointer) {
    ...
    const powerText = this.children.getByData('type', 'powerText') as Phaser.GameObjects.Text;
    if (powerText) {
      powerText.setText(`Power: ${powerPercentage}%`);
      powerText.setPosition(this.cueBall.x, this.cueBall.y - 50);
    }
  }
