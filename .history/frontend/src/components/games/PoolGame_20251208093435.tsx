...
const PoolGame: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initializeGame = () => {
      ...
      // Get game entries - this will need to be fetched separately in real implementation
      const mockEntries: GameEntry[] = [
        ...
        {
          id: 2,
          user_id: currentGame.creator_id === authState.user?.id ? 999 : authState.user?.id || 999,
          game_id: currentGame.id,
          stake_amount: currentGame.stake_amount,
          joined_at: new Date().toISOString(),
          result: null,
          ...
          updated_at: new Date().toISOString(),
          username: 'Player 2',
          user: currentGame.creator_id === authState.user?.id ? undefined : authState.user
        }
      ];
