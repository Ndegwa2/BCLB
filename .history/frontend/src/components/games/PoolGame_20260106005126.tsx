...
class PoolGameScene extends Phaser.Scene {
  ...
  private createBallTexture(x: number, y: number, color: number, isStripe: boolean, number: number, isCue: boolean): Phaser.Physics.Matter.Sprite {
    ...
    for (let i = 6; i > 0; i--) {
      const ratio = i / 6;
      const stepColor = Phaser.Display.Color.ValueToColor(color).lighten(ratio * 15).color;
      graphics.fillStyle(stepColor, 0.4);
    ...
    if (number !== 0) {
      ...
      const numberColor = (number === 8 || isStripe) ? '#000000' : '#000000';
      const textObj = this.add.text(ballRadius, ballRadius, number.toString(), {
        fontSize: `${Math.floor(ballRadius * 0.9)}px`,
        color: numberColor,
        fontStyle: 'bold',
        fontFamily: 'Arial Black, Arial'
      }).setOrigin(0.5);
      
      // Render the text to the graphics
      const textTexture = this.add.renderTexture(0, 0, textureWidth, textureHeight);
      textTexture.draw(textObj, ballRadius, ballRadius);
      textTexture.saveTexture(`ballText_${number}`);
      textObj.destroy();
      textTexture.destroy();
    }
    ...
    graphics.fillCircle(ballRadius - 6, ballRadius - 6, ballRadius * 0.3);
    graphics.fillStyle(0xffffff, 0.3);
    graphics.fillCircle(ballRadius - 6, ballRadius - 6, ballRadius * 0.45);
    
    // Darker edge for definition
    graphics.lineStyle(1.5, 0x000000, 0.2);
    graphics.strokeCircle(ballRadius, ballRadius, ballRadius);
    
    // Generate texture
    const textureKey = `ball_${number}_${Date.now()}`;
