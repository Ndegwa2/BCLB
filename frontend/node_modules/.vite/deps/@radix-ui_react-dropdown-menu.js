...
function createSlot8(ownerName) {
  ...
  const Slot22 = React35.forwardRef((props, forwardedRef) => {
    ...
    if (slottable) {
      ...
      const newChildren = childrenArray.map((child) => {
        if (child === slottable) {
          if (React35.Children.count(newElement) > 1)
            return React35.Children.only(null);
...
function dispatchDiscreteCustomEvent2(target, event) {
  if (target)
    ReactDOM9.flushSync(() => target.dispatchEvent(event));
}
